...
- HTML shell: `www/index.html`
- Styling: `www/style.css`
- Gameplay/runtime logic: `www/script.js`
- Rendering model:
...
- File: `preview.ps1`
- Role:
  - Serves `www/` locally over HTTP as the canonical runtime bundle.
  - Resolves static assets.
  - Exposes `/stats` for persistent local telemetry.
  - Writes `.preview-data/match-stats.json` outside the shipped runtime bundle.
- Why it matters:
  ...
  - Preview-only mutable data should not be stored under `www/`.
...
- Files:
  ...
  - `open-game.ps1`
- Role:
  - `run-game.bat` is the supported local entry point.
  - `open-game.ps1` is the internal launcher helper behind it.
  - The helper finds an open port, starts the preview server, waits for readiness, and opens the game in the default browser.
- Why it matters:
  - There is one user-facing entry point and one internal implementation path.
  - Future launch behavior changes should go into `open-game.ps1`, not duplicated elsewhere.

### Persistent Local Stats

- Files:
  - `.preview-data/match-stats.json`
  - `.gitignore`
- Role:
...
- Why it matters:
  ...
  - The file is ignored in git because it is preview/runtime data, not source.
...
- Working/source art:
  - `Reference/art/`
- Role:
  - `www/assets/` contains the shipped runtime images used by the game.
  - `Reference/art/` holds source or working art files and references.
- Why it matters:
  - Runtime code should reference `www/assets/` through document-relative URLs.
  - `Reference/art/` is useful for future art swaps, but should not be assumed to be live runtime content.

### Deployment Workflow

- File: `.github/workflows/deploy-pages.yml`
- Role:
  - Deploys the `www/` static site to GitHub Pages on pushes to `main`.
- Why it matters:
  ...
  - Pages should publish `./www`, not the repo root.
...
- Role:
  ...
  - `node --check` is used as a fast syntax guard for `www/script.js`.
  - The harness is the main external consumer check for the canonical runtime.
- Why it matters:
...
- Main idea:
  - `state` in `www/script.js` is the central source of truth for gameplay, UI flow, and match progression.
- Why it matters:
...
- Why it matters:
  - We can evolve `.preview-data/match-stats.json` over time.
  - Older local stats files remain usable.

### Shared Launcher Path

- Main idea:
  - Different launch mechanics should converge into one canonical launch pipeline.
- Why it matters:
...
- Main idea:
  - `www/` is the only shipped web runtime, and every consumer should point at it directly.
- Why it matters:
  - Prevents duplicate app roots from drifting apart.
  - Keeps preview, Pages, Capacitor, and harness behavior aligned.
...
- Runtime entry: `www/index.html`
- Main logic: `www/script.js`
- Styling: `www/style.css`
- Runtime assets: `www/assets/`
- Preview server: `preview.ps1`
- Supported launcher: `run-game.bat`
- Internal launcher helper: `open-game.ps1`
- Persistent stats file: `.preview-data/match-stats.json`
- Reference docs and source art: `Reference/`
...
- If we split `www/script.js` into smaller modules
